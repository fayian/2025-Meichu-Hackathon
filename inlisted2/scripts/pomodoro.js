...
class PomodoroTimer {
  ...
  setupEventListeners() {
    ...
    // Custom time setting
    document
      .getElementById("setCustomTimeBtn")
    ...
    const taskImportanceSelect = document.getElementById(
  ...
  setTime(minutes, doConfirm = true) {
    ...
    window.electronAPI.setPomodoroTime(minutes * 60);
  }
  ...
  startTimer() {
    ...
    window.electronAPI.startPomodoro(this.timeLeft);
  }
  ...
  pauseTimer() {
    if (this.isRunning && !this.isPaused) {
      ...
      window.electronAPI.pausePomodoro();
  ...
  stopTimer() {
    ...
    this.isRunning = false;
    this.isPaused = false;
    window.electronAPI.stopPomodoro();
  }
  ...
  showSmartCompletionDialog(sessionResult) {
    ...
    }分鐘 (${breakSuggestion.kind === "short" ? "短" : "長"}休息)\n${
